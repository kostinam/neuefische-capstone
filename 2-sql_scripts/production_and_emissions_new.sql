--CREATE TABLE fao_production_and_emissions_new_2 AS (
	SELECT
		*,
		CASE
			WHEN emissions_co2eq_in_kt IS NULL THEN emissions_co2eq_calc_in_kt
			ELSE emissions_co2eq_in_kt
		END AS emissions_co2eq_combined_in_kt,
		CASE
	...
	FROM
		...
		LEFT JOIN area_mapping am
			ON am.country = t.area
		WHERE
			t.item_group1 = 'no' AND (t.area_group != 'yes' OR t.area = 'World')) AS h
			ORDER BY
		country,
		YEAR,
		item
